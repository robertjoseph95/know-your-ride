...
**Status:** Design approved in chat on 2026-07-16; written specification pending user review
...
The runtime PostgreSQL connection uses Supabase's serverless-appropriate pooled connection mode and a dedicated least-privilege database role. The normal API role receives no arbitrary table access. It may execute only reviewed operations in a non-exposed schema.
...
Supabase secret/service credentials are project-wide and bypass RLS, so "narrow use" in code is not sufficient isolation. They live only in a separate minimal identity-admin worker deployment; ordinary KYR API functions do not receive those environment variables. The worker does not accept a caller-supplied user ID. It claims durable, server-created jobs from a restricted queue and is allowlisted to two Auth administration effects: revoke all sessions for a user and delete an Auth user after the purge gate. Signup, verification, login, refresh, recovery, JWT verification, and normal user-data operations do not use the secret/service credential. Deployment manifests and bundle/environment tests prove the credential is absent from the browser and ordinary API runtime.
...
One successful Free service creation inserts one `-1` entry in the same transaction as the service. A unique relation to the service event prevents duplicate consumption. Soft deletion inserts no refund. Balance is computed from the ledger and may be cached only as a rebuildable projection.

### 5.9 Operational tables

- `stripe_webhook_events` stores event ID, type, Stripe creation time, payload hash, processing status, attempt/lease metadata, a monotonically increasing fencing token, relevant Stripe object IDs, and timestamps. Full payloads are not retained by default.
...
- `migration_records` stores source entity type, a keyed HMAC of any normalized source identity, target ID, checksum, migration revision, and import timestamp. The HMAC key is a separately managed migration secret; plain or unsalted hashes of email addresses are not stored.
- `account_deletion_jobs` is a durable outbox for session revocation, Stripe cancellation, confirmation delivery, final purge, attempts, leases, and last safe error code. Its user foreign key becomes null after Auth deletion; only the keyed deletion identity and any minimum unresolved provider identifier survive until the job reaches a terminal state. Provider side effects are never represented only by an in-memory request.
...
Only the recovery endpoint may return a deletion-pending profile to `active` during the 30-day window. It cancels the pending application purge but not the already-recorded Stripe cancellation intent. After the deadline, the purge runner removes active application data and the Auth identity. If a Stripe operation remains unresolved, the outbox retains only the minimum Stripe identifiers needed to finish cancellation and then removes them under the documented retention policy.
