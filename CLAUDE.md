...
- **ver:0 values are STRIPPED from the shipped payload at build** (C1, 2026-07-02): render-time gating alone is not enough — unverified oil/parts/fluids ship as bare `{ver:0}` shells, unverified torque/maint rows drop, and internal provenance keys (`source`/`last_verified_at`) never ship. A generic build guard hard-fails if ANY `ver:0` object ships value-bearing keys (P1-1, 2026-07-11).
- **Every file that ships to a user must name its generator, and every generator must consume the gate** (2026-07-11): the data blob is NOT the only publication surface — the SEO pages (`wrench_seo.py`), the Pro-guide input (`wrench_deploy/api/specs.json`, regenerated ONLY via `_gen_guide_specs.py` from verified rows; the deploy guard rejects it without the `_meta.generated_by` marker), and hard-coded page samples are all gated surfaces. **Audit SURFACES, not fixes**: verify the property on every output path, against the DB, on the live site — not just on the artifact a fix touched.
